...
function MNISTcfd() {
      ...
      const handleClick = () => {
         setLoading(true);
         axios.get('http://localhost:5000/run3').then (response => {
      ...
      return(
        <div>
          ...
          <button className='btndt' onClick={handleClick}> Train MNIST Dataset </button>
          <ToastContainer/>
         ...
         {loading && <div><h5>Loading...</h5></div>}
          ...
          <h2>The MNIST dataset will be split into two parts and
          ...
          <div className='dt3-container'>
            ...
            <h3>Catastrophic Forgetting Detected!</h3>
              ...
              <h1> Try Test for MNIST Dataset </h1>
            )}
          </div>
