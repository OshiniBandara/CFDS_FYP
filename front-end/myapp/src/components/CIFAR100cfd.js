...
function CIFAR100cfd() {
      ...
      const handleClick = () => {
         setLoading(true);
         axios.get('http://localhost:5000/run2').then (response => {
      ...
      return(
        <div>
          ...
          <button className='btndt' onClick={handleClick}> Train CIFAR 100 Dataset </button>
         ...
         {loading && <div><h5>Loading...</h5></div>}
          ...
          <h2>The Cifar100 dataset will be split into two parts and
           ...
           {/*  <p>{output.length > 0 && output.map((data, index) => {
              if (index ==0) return  <span key={index}>{data}<br /><br /></span>
              return <span key={index}>{data}<br /></span>;
            })}</p> */}
            {dataLoaded ? (
            <div className='dt4-container'>
          ...
          <div className='dt3-container'>
            ...
            <h3>Catastrophic Forgetting Detected!</h3>
              ...
              <h1> Try Test for CIFAR 100 Dataset </h1>
            )}
          </div>
