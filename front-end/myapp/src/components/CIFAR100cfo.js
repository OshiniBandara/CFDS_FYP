...
function CIFAR10cfd() {
  ...
  const handleClick = () => {
    setLoading1(true);
    axios.get('http://localhost:5000/normal2').then (response => {
 ...
 const handleClick2 = () => {
   setLoading2(true);
   axios.get('http://localhost:5000/ssls2').then (response => {
      // Parse the JSON response and access the first element
      const outputString = response.data.output[0];
    ...
    .catch(error => {
      ...
      return(
        <div>
          ...
          <button className='btndt' onClick={handleClick}> Train CIFAR 100 Dataset without SSLS</button>
          <ToastContainer/>
          ...
          <h2>Accuracy for CIFAR100 Dataset without SSLS</h2><br/>
          ...
          <button className='btndt' onClick={handleClick2}> Train CIFAR 100 Dataset with SSLS</button>
          <ToastContainer/>
          ...
          {dataLoaded2 ? (
          <div className='dt2-container'>
          <h2>Accuracy for CIFAR100 Dataset for with SSLS</h2><br/>
          
          {dataLoaded2 && <p>{output2}</p>}
            </div>
            ):(
            <br/>
           )}
